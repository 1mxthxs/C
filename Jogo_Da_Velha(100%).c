...
void jogo_da_velha(char vet[9],char ma[3][3],int vez,int temp,int tipo,char jog1[30],char jog2[30]){	
    int i,l,c,numeric = 7,win,soma[16];
    char n;
...
//////////////VER QUEM GANHOU
	...
	soma[0]=0;
	for(c = 0;c < 3;c++){if(ma[0][c] == X)soma[0]++;}

	soma[1]=0;
	for(c = 0;c < 3;c++){if(ma[1][c] == X)soma[1]++;}

	soma[2]=0;
	for(c = 0;c < 3;c++){if(ma[2][c] == X)soma[2]++;}

	soma[3]=0;
	for(c = 0;c < 3;c++){if(ma[c][0] == X)soma[3]++;}

	soma[4]=0;
	for(c = 0;c < 3;c++){if(ma[c][1] == X)soma[4]++;}
    ...
    l = 2;
	soma[6]=0;
	for(c = 0;c < 3;c++){if(ma[l][c] == X){soma[6]++;l--;}
	}

    soma[7]=0;
	for(c = 0;c < 3;c++){if(ma[c][2] == X)soma[7]++;}

    soma[8]=0;
	for(c = 0;c < 3;c++){if(ma[c][2] == O)soma[8]++;}

	soma[9]=0;
	for(c = 0;c < 3;c++){if(ma[0][c] == O)soma[9]++;}

    soma[10]=0;
	for(c = 0;c < 3;c++){if(ma[1][c] == O)soma[10]++;}
	...
	soma[13]=0;
	for(c = 0;c < 3;c++){if(ma[2][c] == O)soma[13]++;}

	soma[14]=0;
	for(c = 0;c < 3;c++){if(ma[c][0] == O)soma[14]++;}

    soma[15]=0;
	for(c = 0;c < 3;c++){if(ma[c][1] == O)soma[15]++;}
